...
[WarriorJs](https://warriorjs.com/). The premise of the game is to write a
...
tile and your goal is to reach the green tile.

I thought it would be a cool idea to write down how to approach solving a
problem like this, which also turns out to be a good way to explain how I
explore unfamiliar code.

Before we start, one piece of warning...

# SPOILERS AHEAD!

So! I'll start by taking the exposed values and functions from each
top-level module and pasting them in the same file. No docs, no type
signatures, nothing, just their **names**. As [the sage once
...
At this point we clearly don't understand what is really going on with the
code, but just by reading this file a couple of times we are getting
familiar with the _names_ in this particular domain. Just let it soak in.

Now I'm getting more curious and will take a look at the exposed data
constructors: `Direction`, `Action`, and `Item`. I feel that whenever a
module exposes the inner contents of a type, that's an invitation to the
user of that module to pattern match on it, so I'm sure it's going to be
useful to glance over them.
...
The amount of information packed in these simple names is massive, so don't
worry if you feel a bit overwhelmed. Let's move onto the first level.
...
We can see on the right hand side that every 500 milliseconds a `Player
waits` message is being printed.

So we need to import `Direction` and change the `takeTurn` function to:
...
Our warrior can only move right each step, so since the dungeon turns
right, it gets stuck there. Poor fella.
...
What's that `Tile` value? Glad you asked.
...
case expressions doesn't fit the bill any longer. We need our warrior to
try to walk into any possible direction. We can do this by:
...
we're not checking if we already visited the same tile in the past. Let's
take a look at the `History` module:
...
The `Int` that we attached to `EmptyFound` describes how many times we have
...
compareExploration first second =
    if first == second then
        ...
        case ( first, second ) of
            ...
            ( EmptyFound x, EmptyFound y ) ->
                compare x y

            ( EmptyFound _, WallFound ) ->
                LT
...
takeTurn warrior map history =
    let
        ...
        exploreDirection dir =
            case Map.look dir warrior map of
                ( newCoords, Empty ) :: _ ->
                    ( EmptyFound (visitedCount newCoords)
                    , Warrior.Move dir
                    )
...
So [here](https://ellie-app.com/9qFv4Px8MJKa1) is where we are stuck now.
